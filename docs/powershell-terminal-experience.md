...
配置不导入 `posh-git`。仓库已有的 posh-git 按需加载配置保持不变，避免在每次启动 PowerShell 时增加冷启动延迟。
...
1. 从 PSGallery 为当前用户安装 `PSFzf`。
2. 若未找到 `fzf`，通过 `winget` 安装 `junegunn.fzf`。
3. 把配置模板复制到 `$HOME\.config\homekit\powershell\TerminalExperience.ps1`。
4. 在 `$PROFILE.CurrentUserCurrentHost` 中添加或更新一段带 `homekit-terminal-experience` 标记的点加载代码。
...
迁移到另一台 PC 时，克隆或复制本仓库后重新执行安装脚本即可。安装脚本使用 `$PROFILE` 和 `$HOME` 动态确定路径，不依赖特定用户名或盘符。
